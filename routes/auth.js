...
router.post(
    "/register",
    [validate(validation.userSchema)],
    async (req, res) => {
        if (req.body && req.body.email) {
            const user = await User.findOne({ email: req.body.email });
            if (user) {
                return res.status(400).json({
                    response: "Account with Same Email address already exist!",
                });
            }
        }
        if (req.body && req.body.username) {
            ...
            if (username) {
                return res
                    .status(400)
                    .json({ response: "Username already exist!" });
            }
        }
        try {
            ...
            console.log("🚀 ~ file: auth.js:46 ~ err:", err);
...
router.post("/login", [validate(validation.loginSchema)], async (req, res) => {
    ...
    if (req.body && !req.body.username && !req.body.password) {
        return res
            .status(400)
            .json({ response: "Please check the credentials!" });
    }
    try {
        // VALIDATE USERNAME
        ...
        if (!user) {
            return res
                .status(400)
                .json({ response: "Username does not exist!" });
        }
        // VALIDATE PASSWORD
        ...
        if (!validated) {
            return res
                .status(400)
                .json({ response: "Please check the password!" });
        }
        ...
        console.log("🚀 ~ file: auth.js:79 ~ router.post ~ err:", err);
        return res.status(500).json(err);
    }
});
//isLoggedin
router.post("/isLoggedin", async (req, res) => {
    try {
        ...
        } else return res.status(400).json({ response: "Token Expired." });
    } catch (error) {
        console.log(error);
